...
const UPDATES = [
  {
    ...
    firmwareVersion: "3.14.19",
    appVersion: "3.19.6",
    completionDate: "2026-09-19",
    changesZh: [
      "Gentle 模式新增弹窗文案",
      "异常恢复新增脱困失败、碰撞条、行走电机三种场景",
      "4G 稳定性优化",
      "Pause by User 优化",
      "障碍物识别文案修复（Vehicle 误报）"
    ],
    changesEn: [
      "Added a pop-up message for Gentle mode",
      "Added three new recovery-notification scenarios: failed to escape, bumper (collision strip), and drive motor",
      "Improved 4G stability",
      "Improved the \"paused by user\" behavior",
      "Fixed obstacle-recognition wording (false positive for \"Vehicle\")"
    ],
    changesJa: [
      "Gentle モードにポップアップ通知文言を追加",
      "異常復帰に「脱出失敗」「衝突バンパー」「走行モーター」の3つのシナリオを追加",
      "4G の安定性を改善",
      "「Pause by User」の挙動を改善",
      "障害物認識の文言を修正（Vehicle の誤検知）"
    ],
    changesKo: [
      "Gentle 모드에 팝업 안내 문구 추가",
      "이상 복구에 탈출 실패, 범퍼(충돌 바), 주행 모터 세 가지 시나리오 추가",
      "4G 안정성 개선",
      "\"Pause by User\" 동작 개선",
      "장애물 인식 문구 수정(Vehicle 오탐)"
    ],
    changesDe: [
      "Pop-up-Hinweistext für den Gentle-Modus hinzugefügt",
      "Drei neue Szenarien für die Störungswiederherstellung hinzugefügt: Befreiung fehlgeschlagen, Bumper (Kollisionsleiste) und Antriebsmotor",
      "4G-Stabilität verbessert",
      "Verhalten von „Pause by User\" verbessert",
      "Text zur Hinderniserkennung korrigiert (Fehlalarm bei „Vehicle\")"
    ],
    changesFr: [
      "Ajout d'un message contextuel pour le mode Gentle",
      "Ajout de trois nouveaux scénarios de récupération après anomalie : échec de dégagement, pare-chocs (barre de collision) et moteur de déplacement",
      "Amélioration de la stabilité 4G",
      "Amélioration du comportement de « Pause by User »",
      "Correction du texte de reconnaissance d'obstacles (faux positif pour « Vehicle »)"
    ],
    changesIt: [
      "Aggiunto un messaggio popup per la modalità Gentle",
      "Aggiunti tre nuovi scenari di ripristino da anomalie: mancata liberazione, paraurti (barra antiurto) e motore di movimento",
      "Migliorata la stabilità del 4G",
      "Migliorato il comportamento di \"Pause by User\"",
      "Corretto il testo di riconoscimento degli ostacoli (falso positivo per \"Vehicle\")"
    ],
    changesRu: [
      "Добавлено всплывающее сообщение для режима Gentle",
      "Добавлены три новых сценария восстановления после сбоя: не удалось выбраться, бампер (полоса столкновения) и ходовой двигатель",
      "Улучшена стабильность 4G",
      "Улучшено поведение функции «Pause by User»",
      "Исправлен текст распознавания препятствий (ложное срабатывание для «Vehicle»)"
    ]
  },
  {
    id: "r12-hotfix3",
